...
template<typename T1>
std::pair<std::vector<float>,std::vector<std::vector<std::vector<float>>>>  SkimAnalyzer<T1>::HLTAnalyze(const edm::Event& iEvent, const edm::EventSetup& iSetup,std::vector< string> HLTPath,std::vector< string> Seed ){
...
template<typename T1>
std::vector<float> SkimAnalyzer<T1>::SelectTrg_Object(std::vector<std::vector<float>> &tr1,std::vector<std::vector<float>> &tr2, std::vector<std::vector<float>> &tr3,std::vector<std::vector<float>> &tr4,std::vector<std::vector<float>> &tr5,std::vector<std::vector<float>> &tr6){
...
template<typename T1> 
std::vector<float> SkimAnalyzer<T1>::SimulateTrigger(std::vector<float> & genMu_pt,std::vector<float> & genMu_eta,std::vector<float> & genMu_phi,std::vector<float> & genMu_ch){
  std::vector<std::vector<float>> max1;
...
if (!UseOnlyBKeeMCForTriplets){
 ...
 //  auto skata=std::make_shared<reco::Track>(tracks->at(trk-tracks->begin()));
//   reco::TrackRef trackref(tracks,trk-tracks->begin());
   // Epropagation ecal(std::make_shared<reco::Track>(tracks->at(trk-tracks->begin())),clusters,bFieldHandle);
 //  if(ecal.success()) cout<<"here"<<endl;
